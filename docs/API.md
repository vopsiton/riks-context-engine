...
>
> ⚠️ **The HTTP API section of this file is auto-generated** from the OpenAPI
> spec (`GET /openapi.json` / `app.openapi()`) by
> `scripts/generate_api_md.py` (#141, issue #123 turn 3) — do not edit that
> section by hand; regenerate with
> `uv run python scripts/generate_api_md.py --write`. The remaining sections
> (installation, package modules, CLI, configuration, exceptions) are manual
> documentation.
>
> The former `docs/API_REFERENCE.md` was merged here and removed (issue #125).

![REST API request flow: client to FastAPI endpoints to memory stores](images/api-flow.png)
...
> **This entire section is auto-generated** from the OpenAPI spec
> (`GET /openapi.json` / `app.openapi()`) by
> `scripts/generate_api_md.py` (#141, issue #123 turn 3). Do not edit it by
> hand — regenerate with `uv run python scripts/generate_api_md.py --write`.
>
...
<!-- AUTO:HTTP-API:BEGIN (generated by scripts/generate_api_md.py — do not edit this section) -->

<!-- Oluşturuldu: 2026-08-19 16:52:28 UTC — `scripts/generate_api_md.py` tarafından otomatik üretilmiştir, manuel düzenleme yapmayın. -->

## HTTP API (FastAPI server) — Rik's Context Engine API v0.4.0

> This section is **auto-generated** from the OpenAPI spec (`GET /openapi.json` / `app.openapi()`). Do not edit it by hand — regenerate with `uv run python scripts/generate_api_md.py --write`.
...
| GET | `/api/v1/context/messages` | Context List Messages |
| GET | `/api/v1/context/summary` | Context Summary |
| GET | `/api/v1/memory/export` | Export Memory Api |
| GET | `/health` | Health |
| GET | `/metrics` | Metrics |
| GET | `/models` | List Models |
| POST | `/api/chat` | Chat |
| POST | `/api/v1/audit/operation` | Audit Record Operation |
| POST | `/api/v1/context/messages` | Context Add Message |
| POST | `/api/v1/memory/import` | Import Memory Api |
...
```json
{
  "message": "Merhaba, staging deploy ne durumda?"
}
```
...
| `category` | `str` | no | Filter by category (e.g. memory.import) |
| `endpoint` | `str` | no | Filter by endpoint path |
...
- `limit`: `int` *(required)*
- `offset`: `int` *(required)*
- `entries`: `list[AuditEntryResponse]` *(required)*
```


#### `POST /api/v1/audit/operation`
...
| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `category` | `string` | yes | Operation category (e.g. context.clear) |
| `endpoint` | `string` | yes | Endpoint path the operation touched |
...
```
**ContextAddRequest**
- `role`: `str` — user | assistant | system (default `"user"`)
- `content`: `str` *(required)* — Message content
...
- `current_tokens`: `int` *(required)*
- `max_tokens`: `int` *(required)*
- `messages_count`: `int` *(required)*
- `active_messages_count`: `int` *(required)*
- `pruning_count`: `int` *(required)*
- `last_prune_timestamp`: `str | None` *(required)*
```
...
| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `types` | `str` | no | Comma-separated types: episodic,semantic,procedural |
| `format` | `str` | no | Output format |
| `date_from` | `str` | no |  |
| `date_to` | `str` | no |  |
| `tags` | `str` | no | Comma-separated tags filter |
...
**MemoryImportRequest**
- `content`: `str` *(required)* — JSON or YAML manifest content
- `format`: `str` (default `"json"`; one of ['json', 'yaml'])
- `merge`: `bool` — If true, skip duplicate IDs; if false, replace all (default `true`)
```
...
```
**HealthResponse**
- `status`: `str` — Liveness state (default `"ok"`)
```
...
```
**ModelsResponse**
- `models`: `list[str]` *(required)* — Available chat models
```
