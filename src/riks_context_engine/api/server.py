...
class MemoryExportRequest(BaseModel):
    types: list[Literal["episodic", "semantic", "procedural"]] | None = None
...
def export_memory_api(
    ...
    if format is None:
        format = "json"
    mem_format = MemoryFormat(format)
    ...
    serialized = dump_manifest(manifest, mem_format)
    counts = {
...
def import_memory_api(req: MemoryImportRequest) -> MemoryImportResponse:
    ...
    mem_format = MemoryFormat(req.format)
    try:
        manifest = parse_manifest(req.content, mem_format)
