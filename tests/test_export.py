...
class TestDumpAndParse:
    def test_json_round_trip(self):
        ...
        manifest = export_memory(ep, sm, pm)
        json_str = dump_manifest(manifest, MemoryFormat.JSON)
        parsed = parse_manifest(json_str, MemoryFormat.JSON)
    ...
    def test_yaml_round_trip(self):
        ...
        yaml_str = dump_manifest(manifest, MemoryFormat.YAML)
        parsed = parse_manifest(yaml_str, MemoryFormat.YAML)
    ...
    def test_schema_version_check_rejects_mismatched(self):
        ...
        with pytest.raises(ValueError, match="Schema version mismatch"):
            parse_manifest(json.dumps(bad_data), MemoryFormat.JSON)

    def test_parse_rejects_non_object(self):
        with pytest.raises(ValueError, match="Expected object"):
            parse_manifest("[]", MemoryFormat.JSON)
...
class TestImportToMemory:
    def test_import_merge_skips_duplicates(self):
        ...
        # merge=True: skips duplicates, only adds new entries
        assert imported["episodic"] == 1
        # ep had 1, ep2 had 1 (different ID), after import ep has 2
    ...
    def test_import_replace_clears_existing(self):
        ...
        # merge=False: clears existing then imports
        assert imported["episodic"] == 1
